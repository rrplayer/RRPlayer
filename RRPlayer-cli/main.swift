...
let iinaPath = execURL.deletingLastPathComponent().appendingPathComponent("RRPlayer").path

guard FileManager.default.fileExists(atPath: iinaPath) else {
  print("Cannot find RRPlayer binary. This command line tool only works in RRPlayer.app bundle.")
  exit(1)
}
...
if userArgs.contains(where: { $0 == "--help" || $0 == "-h" }) {
  print(
    """
    Usage: RRPlayer-cli [arguments] [files] [-- mpv_option [...]]
